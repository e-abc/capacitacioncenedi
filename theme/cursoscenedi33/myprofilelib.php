 ...
 * @return bool
 */
function core_myprofile_navigation_cenedi(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
        if (($iscurrentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('moodle/user:update',
            ...
            if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                ...
                if (empty($url)) {
                    if (empty($course)) {
                        $url = new moodle_url('/theme/cursoscenedi33/editprofile.php', array('id' => $user->id, 'returnto' => 'profile'));
                    } else {
                        $url = new moodle_url('/theme/cursoscenedi33/editprofile.php', array('id' => $user->id, 'course' => $course->id,
                            'returnto' => 'profile'));
                    }
                }
