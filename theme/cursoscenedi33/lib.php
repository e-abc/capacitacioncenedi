 ...
 * @param type|string theme_config $theme The theme config object.
 * @return string The parsed CSS The parsed CSS.
 */
function theme_cursoscenedi33_get_pre_scss($theme) {
    $scss = theme_cursoscenedi33_set_fontwww($css);
    return $scss;
}
 ...
 * @return type|string
 */
function theme_cursoscenedi33_get_extra_scss($theme) {
 ...
 * @return string
 */
function theme_cursoscenedi33_get_main_scss_content($theme) {
    global $CFG;
    $scss = '';
    $parentconfig = theme_config::load('boost');
    $scss .= theme_boost_get_main_scss_content($parentconfig);
    $themescssfile = $CFG->dirroot.'/theme/cursoscenedi33/scss/preset/theme.scss';
 ...
 */
function theme_cursoscenedi33_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/cursoscenedi33/javascript/theme.js');
}
...
function theme_cursoscenedi33_process_css($css, $theme) {
    global $OUTPUT, $CFG;
    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_cursoscenedi33_pre_css_set_fontwww($css);
    // Set custom CSS.
 ...
 *
 * @param type|string $css The CSS.
 * @param type|string $logo The URL of the logo.
 * @return type|string The parsed CSS
 */
function theme_cursoscenedi33_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_cursoscenedi33_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('cursoscenedi33');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        if ($filearea === 'logo') {
            ...
            theme_cursoscenedi33_serve_css($args[1]);
 ...
 * @return string
 */
function theme_cursoscenedi33_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/cursoscenedi33/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/cursoscenedi33/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_cursoscenedi33_send_unmodified($lastmodified, $etagfile);
    }
    theme_cursoscenedi33_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 */
function theme_cursoscenedi33_send_unmodified($lastmodified, $etag) {
...
/**
 *  Cached css for theme_cursoscenedi33
 *
 ...
 * @param type|string $etag
 */
function theme_cursoscenedi33_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_cursoscenedi33_set_customcss($css, $customcss) {
 ...
 */
function theme_cursoscenedi33_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 * @return type|string
 */
function theme_cursoscenedi33_pre_css_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('cursoscenedi33');
    $css = str_replace($tag, $themewww.'/cursoscenedi33/fonts/', $css);
    return $css;
}
 ...
 * @return type|string
 */
function theme_cursoscenedi33_set_fontwww($css) {
    global $CFG, $PAGE;
    if (empty($CFG->themewww)) {
        $themewww = $CFG->wwwroot."/theme";
    } else {
    ...
    $theme = theme_config::load('cursoscenedi33');
    $fontwww = '$fontwww: "'.$themewww.'/cursoscenedi33/fonts/"'.";\n";
    return $css;
 ...
 * @return type|string
 */
function theme_cursoscenedi33_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;
    $nos = theme_cursoscenedi33_get_setting('numberofslides');
    $i = $p % 3;
    $slideimage = $OUTPUT->image_url('home/slide'.$i, 'theme');
    // Get slide image or fallback to default.
    if (theme_cursoscenedi33_get_setting($sliname)) {
 ...
 * @return type|string
 */
function theme_cursoscenedi33_get_setting($setting, $format = false) {
    global $CFG;
 ...
 * @return type|string
 */
function  theme_cursoscenedi33_infolink() {
