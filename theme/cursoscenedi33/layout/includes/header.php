...
//nuevo requerimiento del 20171129
//los usuarios alumnos no deben ver el menú izquierdo ni el botón para activarlo
//los usuarios administradores y los usuarios con rol global con permiso de edición deben ver el menú.
$shownavdrawer = false;
$navdraweropen = false;

//busco  usuarios con rol profesor con permiso de edición a nivel global
global $DB, $USER;
$isprofesor = false;
$profesorsobj = $DB->get_records('role_assignments',array('roleid'=>9,'contextid'=>1));
if($profesorsobj && count($profesorsobj)>0){
	foreach ($profesorsobj as $profesor) {
		if($USER->id == $profesor->userid){
			$isprofesor = true;
		}
	}
}

if ((isloggedin() && $isprofesor) or (is_siteadmin())) {
    $shownavdrawer = true;
    $navdraweropen = true;
}
